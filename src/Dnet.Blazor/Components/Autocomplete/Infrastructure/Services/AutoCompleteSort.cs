...
public class AutoCompleteSort<TGridItem>
{
    ...
    private Func<IQueryable<TGridItem>,IOrderedQueryable<TGridItem>> _first;

    private List<Func<IOrderedQueryable<TGridItem>, IOrderedQueryable<TGridItem>>>? _then;
    ...
    internal AutoCompleteSort(Func<IQueryable<TGridItem>, IOrderedQueryable<TGridItem>> first, (LambdaExpression, bool) firstExpression)
    {
        _first = first;
    ...
    /// <summary>
    /// Produces a <see cref="AutoCompleteSort{T}"/> instance that sorts according to the specified <paramref name="expression"/>, ascending.
    /// </summary>
    ...
    /// <returns>A <see cref="AutoCompleteSort{T}"/> instance representing the specified sorting rule.</returns>
    public static AutoCompleteSort<TGridItem> ByAscending<U>(Expression<Func<TGridItem, U>> expression)
        => new AutoCompleteSort<TGridItem>((queryable) => queryable.OrderBy(expression),
            (expression, true));

    /// <summary>
    /// Produces a <see cref="AutoCompleteSort{T}"/> instance that sorts according to the specified <paramref name="expression"/>, descending.
    /// </summary>
    /// <typeparam name="U">The type of the expression's value.</typeparam>
    /// <param name="expression">An expression defining how a set of <typeparamref name="TGridItem"/> instances are to be sorted.</param>
    /// <returns>A <see cref="AutoCompleteSort{T}"/> instance representing the specified sorting rule.</returns>
    public static AutoCompleteSort<TGridItem> ByDescending<U>(Expression<Func<TGridItem, U>> expression)
        => new AutoCompleteSort<TGridItem>((queryable) =>queryable.OrderByDescending(expression),
            (expression, false));

    /// <summary>
    /// Updates a <see cref="AutoCompleteSort{T}"/> instance by appending a further sorting rule.
    /// </summary>
    /// <typeparam name="U">The type of the expression's value.</typeparam>
    /// <param name="expression">An expression defining how a set of <typeparamref name="TGridItem"/> instances are to be sorted.</param>
    /// <returns>A <see cref="AutoCompleteSort{T}"/> instance representing the specified sorting rule.</returns>
    public AutoCompleteSort<TGridItem> ThenAscending<U>(Expression<Func<TGridItem, U>> expression)
    {
        _then ??= new();
    ...
    /// <param name="expression">An expression defining how a set of <typeparamref name="TGridItem"/> instances are to be sorted.</param>
    /// <returns>A <see cref="AutoCompleteSort{T}"/> instance representing the specified sorting rule.</returns>
    public AutoCompleteSort<TGridItem> ThenDescending<U>(Expression<Func<TGridItem, U>> expression)
    {
        _then ??= new();
        _thenExpressions ??= new();
    ...
    internal IOrderedQueryable<TGridItem> Apply(IQueryable<TGridItem> queryable)
    {
